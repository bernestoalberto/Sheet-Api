 ...
 * @constructor
 */
let SheetsHelper = function(accessToken) {
    let authClient = new googleAuth();
    let auth = new authClient.OAuth2();
    auth.credentials = {
...
SheetsHelper.prototype.createSpreadsheet = function(title, callback) {
    ...
    self.service.spreadsheets.create(request, function(err, spreadsheet) {
        ...
        // Add header rows.
        let dataSheetId = spreadsheet.sheets[0].properties.sheetId;
        let requests = [
            buildHeaderRowRequest(dataSheetId),
        ];
        // Add pivot table and chart.
        let pivotSheetId = spreadsheet.sheets[1].properties.sheetId;
