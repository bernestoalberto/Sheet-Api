...
let env = (process.env.COMPUTERNAME != "ACS-EBONET") ?  'development':'production';
// let dbconn = (env == 'production') ? config.production : config.dev;
let dbconn = config.dev;
// let pool = mysql.createPool(config.database.mysql);
const pool = mysql.createConnection(dbconn);
